...
export default {
  getCotentHome: async (controladora, lng = "pt") => {
    let { data: json } = await api.get(`/content/${controladora}/${lng}`);
    return json;
  },

  getCotentId: async (id, lng = "pt") => {
    let { data: json } = await api.get(`/content/single/${id}/${lng}`);
    return json;
  },

  updateContent: async (titulo, description, id, token, lng = "pt") => {
    ...
    let { data: json } = await api.post(`/content/home/${id}/${lng}`, body);
