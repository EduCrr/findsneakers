...
export default {
  ...
  getTeste: async (page = 1) => {
    let { data: json } = await api.get(`/teste?page=${page}`);
    return json;
  },
  ...
  getData: async (page = 1, link, cat = "0", lng = "pt") => {
    if (link === "posts") {
      let { data: json } = await api.post(
        `/private/posts/${lng}?page=${page}`,
        {
          cat,
        }
      );
      ...
      let { data: json } = await api.post(
        `/private/products/${lng}?page=${page}`,
        {
          cat,
        }
      );
  ...
  getDataOrder: async (link, cat = "0", lng = "pt") => {
    if (link === "posts") {
      let { data: json } = await api.get(`/posts/order/${cat}/${lng}`);
      return json;
    } else if (link === "produtos") {
      let { data: json } = await api.get(`/products/order/${cat}/${lng}`);
      return json;
    }
  },

  search: async (q, link, lng = "pt") => {
    if (link === "posts") {
      let { data: json } = await api.get(`/search/posts/${lng}?q=${q}`);
      return json;
    } else if (link === "produtos") {
      let { data: json } = await api.get(`/search/products/${lng}?q=${q}`);
      return json;
    }
  },
