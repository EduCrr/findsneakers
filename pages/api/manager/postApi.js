...
export default {
  ...
  getPostsPrivate: async (cat = 0, page = 1, lng = "pt") => {
    let { data: json } = await api.post(`/private/posts/${lng}?page=${page}`, {
      cat,
    });
    return json;
  },
  ...
  updatePost: async (
    ...
    let { data: json } = await api.post(`/post/edit/${id}/${lng}`, body);
  ...
  getSinglePost: async (id, lng = "pt") => {
    let { data: json } = await api.get(`/post/private/${id}/${lng}`);
    return json;
  },
