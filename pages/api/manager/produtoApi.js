...
export default {
  getProductsPrivate: async (cat = 0, page = 1, lng = "pt") => {
  ...
  getSingleProduct: async (id, lng = "pt") => {
    let { data: json } = await api.get(`/product/private/${id}/${lng}`);
    return json;
  },
  ...
  updateProduct: async (
    ...
    let { data: json } = await api.post(`/product/edit/${id}/${lng}`, body);
